...
public class Visualtasks extends BaseGameActivity implements IOnSceneTouchListener, IScrollDetectorListener, IPinchZoomDetectorListener, IHoldDetectorListener{
	// ===========================================================
    // Constants
    // ===========================================================
    ...
    public void onLoadResources() {
			...
			this.mFont = new Font(this.mFontTexture, Typeface.create(Typeface.DEFAULT, Typeface.BOLD), 56, true, Color.WHITE);
			
			this.mBackgroundTextureAtlas = new BitmapTextureAtlas(2048, 1024, TextureOptions.BILINEAR_PREMULTIPLYALPHA);
			...
			this.mBackgroundTextureRegion = BitmapTextureAtlasTextureRegionFactory.createFromAsset(this.mBackgroundTextureAtlas, this, "achtergrond1.png", 0, 0);
	        this.mEngine.getTextureManager().loadTextures(this.mBitmapTextureAtlas, this.mFontTexture);
    ...
    public Scene onLoadScene() {
    		...
    		//Sprite backgroundSprite = new Sprite(0,0,this.CAMERA_WIDTH, this.CAMERA_HEIGHT, mBackgroundTextureRegion);
    ...
    public void spawnTask(String name, float pX, float pY){
        ...
        final TaskSprite sprite = new  TaskSprite(0, 0, mFont, mTaskTextureRegion);
        sprite.setPosition(pX-sprite.getWidth()/2f, pY-sprite.getHeight()/2f);
    ...
    private void showPopUp(){
         ...
         final int centerY = (CAMERA_HEIGHT - this.mTaskTextureRegion.getHeight()) / 2;
    	 ...
    	 helpBuilder.setPositiveButton("Ok",
    	   new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
    	    	...
    	    	//Toast.makeText(example, "Title is: " + name, Toast.LENGTH_LONG).show();
    	    }
    	   });
    ...
    class TaskSprite extends Sprite {
		...
		public TaskSprite(float pX, float pY, Font pFont, TextureRegion pTextureRegion) {
			super(pX, pY, pTextureRegion);
			this.mFont = pFont;
			this.mText = new ChangeableText(0, 0, pFont, "Task", HorizontalAlign.CENTER, MAX_CHARACTERS);
    	...
    	public boolean onAreaTouched(TouchEvent pSceneTouchEvent,float pTouchAreaLocalX, float pTouchAreaLocalY) {
    		...
    		case TouchEvent.ACTION_DOWN:
					synchronized(touchingID){
						if(!isTouched){
							...
							this.mScaleX = this.getScaleX(); this.mScaleY = this.getScaleY();
							this.setScaleX(1.1f * mScaleX);this.setScaleY(1.1f * mScaleY);  
    		...
    		case TouchEvent.ACTION_MOVE:
				synchronized(touchingID){
					// Test if zooming
					if(touchingID != INVALID_TOUCHING_ID && touchingID2 != INVALID_TOUCHING_ID){
						float diffStartX = Math.abs(dX-dX2);
						float diffStartY = Math.abs(dY-dY2);
						float startDist = (float)Math.sqrt(diffStartX*diffStartX + diffStartY*diffStartY); //calculate distance
						float currDist = (float)Math.sqrt(diffStartX*diffStartX + diffStartY*diffStartY); //calculate distance
					} else if(touchingID == pSceneTouchEvent.getPointerID()){
						setPosition(pSceneTouchEvent.getX()-dX, pSceneTouchEvent.getY()-dY);
					}
				};
				break;
    		...
    		case TouchEvent.ACTION_CANCEL:
    			synchronized (touchingID){
					if(isZooming && pSceneTouchEvent.getPointerID() == touchingID){
						isZooming = false;
						...
						dX=dX2;
						dY=dY2;
					} else if(isZooming && pSceneTouchEvent.getPointerID() == touchingID2){
						isZooming = false;
						touchingID2= INVALID_TOUCHING_ID;
						
					} else if(isTouched && pSceneTouchEvent.getPointerID() == touchingID){
						touchingID= INVALID_TOUCHING_ID;
						isTouched = false;
						this.setScaleX(mScaleX);this.setScaleY(mScaleY);  
					}
		...
		public void setText(String pText) {
			this.mText.setText(getNormalizedText(mFont, pText, this.getWidth()-2*TEXT_PADDING));
...
//						if (arg0 instanceof TaskSprite && arg1 instanceof TaskSprite){
...
@Override
public void onHold(HoldDetector arg0, long arg1, final float arg2, final float arg3) {
	
	
}
@Override
public void onHoldFinished(HoldDetector arg0, long arg1, final float arg2, final float arg3) {
